...
export default function MetricsCards({ currentYearResult }) {
    ...
    const severityScale = 0.5; // 50% Paralysis Deceleration Factor
            ...
            <div className={`md:col-span-5 flex flex-col p-5 border bg-emerald-500/5 text-emerald-800 border-emerald-200 backdrop-blur-sm shadow-sm transition-all hover:shadow-md relative group ${showImpact ? 'min-h-[420px]' : 'h-32 justify-center'}`}>
                ...
                {showImpact && (
                    <div className="mt-2 pt-4 border-t border-emerald-200/50 flex-1 animate-in fade-in slide-in-from-top-4 duration-300">
                            ...
                            {impactPage === 0 ? (
                                <>
                                    <div className="flex items-start gap-3 bg-white/40 p-2 rounded">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-bold text-slate-700">{Math.round(govDays * 34 * severityScale).toLocaleString()} km <span className="font-normal opacity-70">of 4-lane National Highways</span></h4>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-3 bg-white/40 p-2 rounded">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-bold text-slate-700">{Math.round(govDays * 14 * severityScale).toLocaleString()} km <span className="font-normal opacity-70">of Railway Track (DFC Norms)</span></h4>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-3 bg-white/40 p-2 rounded">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-bold text-slate-700">{Math.round(govDays * 11000 * severityScale).toLocaleString()} <span className="font-normal opacity-70">PM Awas Yojana Housing Units</span></h4>
                                        </div>
                                    </div>
                                </>
                            ) : (
                                <>
                                    <div className="flex items-start gap-3 bg-white/40 p-2 rounded">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-bold text-slate-700">{Math.round(govDays * 73 * severityScale).toLocaleString()} <span className="font-normal opacity-70">Ayushman Health & Wellness Centers</span></h4>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-3 bg-white/40 p-2 rounded">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-bold text-slate-700">{Math.round(govDays * 75000 * severityScale).toLocaleString()} <span className="font-normal opacity-70">Jal Jeevan Water Connections</span></h4>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-3 bg-white/40 p-2 rounded">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-bold text-slate-700">{Math.round(govDays * 33 * severityScale).toLocaleString()} MW <span className="font-normal opacity-70">of Solar Power Infra Setup</span></h4>
                                        </div>
                                    </div>
                                </>
                            )}
                        ...
                        <p className="mt-4 text-[9px] text-emerald-600/60 italic text-center leading-tight">
                            Based on verified 2023-24 daily averages (MoRTH, Indian Railways).<br />
                            <span className="opacity-75">*Conservatively assumes 50% deceleration of physical output during MCC due to policy paralysis.</span>
                        </p>
                    </div>
                )}
